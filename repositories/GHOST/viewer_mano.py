...
from gaussian_renderer import GaussianModelManoHybrid, render
...
class LocalViewer(Mini3DViewer):
    ...
    def init_gaussians(self):
        # load gaussians
        self.gaussians = GaussianModelManoHybrid(self.cfg.sh_degree)
        unselected_fid = []
            ...
            with dpg.window(label="MANO parameters", tag="_mano_window", autosize=True, pos=(self.W-300, 0)):
                def callback_enable_control(sender, app_data):
                    if app_data:
                        ...
                        self.gaussians.update_mesh_by_param_dict(self.pose_param, self.shape_param)
                    else:
                        self.gaussians.select_mesh_by_timestep(self.timestep)
                    self.need_update = True
                ...
                def callback_set_pose(sender, app_data):
                    joint, axis = sender.split('-')[1:3]
                    axis_idx = {'x': 0, 'y': 1, 'z': 2}[axis]
                    self.pose_param.view(16, 3)[int(joint[1:]), axis_idx] = app_data
                    if not dpg.get_value("_checkbox_enable_control"):
                        dpg.set_value("_checkbox_enable_control", True)
                    self.gaussians.update_mesh_by_param_dict(self.pose_param, None)
                    self.need_update = True

                dpg.add_text(f'Joints')
                self.pose_sliders = []
                max_rot = 2.0
                # 16 hand joints named J0 to J15
                joint_names = [f'J{i}' for i in range(16)]
                # joint_names = ['neck', 'jaw', 'eyes']
                for i, joint in enumerate(joint_names):
                    # if joint in self.flame_param:
                        with dpg.group(horizontal=True):
                            dpg.add_slider_float(min_value=-max_rot, max_value=max_rot, format="%.2f", default_value=self.pose_param.view(16, 3)[i, 0], callback=callback_set_pose, tag=f"_slider-{joint}-x", width=70)
                            dpg.add_slider_float(min_value=-max_rot, max_value=max_rot, format="%.2f", default_value=self.pose_param.view(16, 3)[i, 1], callback=callback_set_pose, tag=f"_slider-{joint}-y", width=70)
                            dpg.add_slider_float(min_value=-max_rot, max_value=max_rot, format="%.2f", default_value=self.pose_param.view(16, 3)[i, 2], callback=callback_set_pose, tag=f"_slider-{joint}-z", width=70)
                ...
                def callback_set_shape(sender, app_data):
                    shape_i = int(sender.split('-')[2])
                    self.shape_param[0, shape_i] = app_data
                    if not dpg.get_value("_checkbox_enable_control"):
                        dpg.set_value("_checkbox_enable_control", True)
                    self.gaussians.update_mesh_by_param_dict(None, self.shape_param)
                    self.need_update = True

                self.shape_sliders = []

                dpg.add_text(f'Shape')
                for i in range(10):
                    dpg.add_slider_float(label=f"{i}", min_value=-3, max_value=3, format="%.2f", default_value=0, callback=callback_set_shape, tag=f"_slider-expr-{i}", width=250)
                    self.shape_sliders.append(f"_slider-expr-{i}")

                def callback_reset_mano(sender, app_data):
                    self.reset_param()
                    if not dpg.get_value("_checkbox_enable_control"):
                        dpg.set_value("_checkbox_enable_control", True)
                    self.gaussians.update_mesh_by_param_dict(None, None)


                # dpg.add_button(label="reset FLAME", tag="_button_reset_flame", callback=callback_reset_flame)
                dpg.add_button(label="reset MANO", tag="_button_reset_flame", callback=callback_reset_mano)
        ...
        while dpg.is_dearpygui_running():
            ...
            if self.need_update or self.playing:
                ...
                if dpg.get_value("_checkbox_show_splatting"):
                    ...
                    # self.gaussians.save_ply(self.cfg.save_folder / f"{time.strftime('%Y-%m-%d_%H-%M-%S')}.ply")
                    # opacity
