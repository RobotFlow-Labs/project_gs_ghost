...
def psnr_masked(img1, img2, mask):
    #mse=0 is avoided by skipping images with nothing to render (empty mask)
    if mask.sum() == 0:
        raise ValueError("Should not be running psnr_masked with all zeros mask!")
    
    #compute division factor including RGB channels
    if len(mask.shape) == 3:
        channel_position = 0#no batch
    elif len(mask.shape) == 4:
        channel_position = 1#with batch
    else:
        raise ValueError("Unhandled mask shape!")
    if mask.shape[channel_position] == 1:
        valid_pixels = mask.sum() * 3
    elif mask.shape[channel_position] == 3:
        valid_pixels = mask.sum()

    diff = (img1*mask) - (img2*mask)
    mse = (diff ** 2).view(img1.shape[0], -1).sum(1, keepdim=True)/valid_pixels#[1, 1] shape. Needs .mean() on return value
    return 20 * torch.log10(1.0 / torch.sqrt(mse))
